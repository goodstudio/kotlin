...
private class JsStringConcatenationTransformer(val context: CommonBackendContext) : IrElementTransformerVoid() {
    private val IrType.shouldExplicitlyConvertToString: Boolean
        get() {
             ...
             * We will unwrap the toString() call later in [JsStringConcatenationOptimizationLowering], only for production builds.
             *
    ...
    private fun IrExpression.explicitlyConvertedToString(): IrExpression {
        ...
        return if (type.isNullable()) {
            JsIrBuilder.buildCall(context.ir.symbols.extensionToString, origin = JsStatementOrigins.EXPLICIT_TO_STRING_CALL).apply {
            ...
            JsIrBuilder.buildCall(toStringMethodSymbol, origin = JsStatementOrigins.EXPLICIT_TO_STRING_CALL).apply {
