...
private val annotationTargetsOnErrorAnnotation: Set<KotlinTarget> = KotlinTarget.entries.toMutableSet().apply {
    remove(KotlinTarget.INITIALIZER)
    remove(KotlinTarget.DESTRUCTURING_DECLARATION)
}

fun FirAnnotation.getAllowedAnnotationTargets(session: FirSession): Set<KotlinTarget> {
    if (annotationTypeRef is FirErrorTypeRef) return annotationTargetsOnErrorAnnotation
